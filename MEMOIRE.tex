...
First of all, we write the \emph{i}-th \emph{order statistics} $X_{(i)} $ which are the statistics ordered by increasing value 
...
\subsection*{Some Definitions and ..}
\theoremstyle{definition}
...
\newline
For these two diagnostic tools, we use the order statistics (see eq.(\ref{ordereds} in appendix) but now we rather consider an \textbf{ordered sample} of independent observations :
...
\part{Experimental Framework : Global....}
\thispagestyle{empty}
...
\chapter{Introduction to the Analysis}
\minitoc \thispagestyle{empty}
...
\begin{itemize}
\item We fitted a simple GAM model relying on the \texttt{mgcv} package from \citet{maindonald_data_2006}. Then, after looking at the correlation structure the normalized residuals (ACF and PACF, see figure \ref{fig:acfresgam1} in \hyperref[app:fig]{appendix \ref{app:fig}}), we detected very slight patterns and hence we wanted to allow taking this into account.

\item As the selection from figure \ref{fig:acfresgam1} is not trivial, we fitted several time series models for the residuals. It is not necessary to consider too complex models. The results are represented in table \ref{table:gamresid} in \hyperref[app:fig]{appendix \ref{app:fig}} where we see that the BIC, which is more penalizing complex models, will prefer the independent model for the residuals but the AIC will not. Hence, we conducted likelihood ratio tests to compare these models which let us favour the MA(1) model for the residuals. The diagnostics of the model in figure \ref{fig:diagnogam} are good. As we took a Gaussian (identity) link, our model can be written

\begin{equation}\label{eq:gam}
Y_{\text{GAM}}(\text{year}) = f_0 + f_{\text{1}}(\text{year})+\epsilon , \qquad\quad \epsilon\sim\text{MA(1)}.
\end{equation}
where we choose to model $f$ by splines. $Y$ models the TX here. 

\item  To better account for uncertainty, we decide to simulate draws from the posterior of the GAM model in (\ref{eq:gam}). We (($10^5$ here; it is fast) 
We display 50 of the posterior draws in figure \ref{fig:post_draws} together with 

Hence, the confidence intervals (or bands) that we will compute will be of the form of  Bayesian credible intervals that we have discussed in \hyperref[bayes_cred_int]{section \textbf{\ref{bayes_cred_int}}}. 


We can see from figure \ref{fig:center_gam} the two significant increase in trend. the decrease we have pointed out in the preceding section with LOESS is not significant. However, we will see that this is not the case when doing the correction for simultaneous intervals. 
\end{itemize}
...
\begin{figure}[!htb]
	\includegraphics[width=.99\linewidth]{splines.pdf}\caption{Plots of the first derivative \big($f^{'}(\cdot)$\big) of the estimated splines on the retained GAM model. Grey area represents the $95\%$ confidence interval. Sections of the spline where the confidence interval does not include zero are indicated by thicker sections. }\label{fig:derivsplines}
\end{figure}

Some remarks from the two plots of figure \ref{fig:derivsplines} :
...
\item  Whereas the pointwise confidence interval include significant regions (when 0 is not included in the interval), the simultaneous interval which accounting for the increase in uncertainty, have no more significant regions. We can concluce that the 
...
\section{Comments and Structure of the Analysis }
 ...
 We have remarked that there is indeed an upward (linear) trend for the series of yearly maxima.
We have then remarked that the \textbf{nonstationary} component is present. Hence, after having presented a stationary analysis in \hyperref[sec:anagev]{section \ref{sec:anagev}}, we will make an in-depth nonstationary analysis in \hyperref[sec:ananonsta]{section \ref{sec:ananonsta}}.

After having... we will now go through with the more specific subjec of this thesis, that is the extreme value analysis.

POT and GEV : 
As you have seen, the analysis in POT or in GEV involves different methods and different data. For the kind of this text, we decide not to display the results of the POT analysis to keep the text not too enormous. Henceforth, we will be able to focus on the GEV analysis. But\textbf{ note that a huge amount of the analysis for POT has been done and is available on the same \hyperref[xxx]{github repository} presented above}.
...
\chapter{Analysis by GEV }\label{sec:anagev}
\minitoc \thispagestyle{empty}
...
\chapter{Nonstationary Analysis}\label{sec:ananonsta}
\minitoc \thispagestyle{empty}
...
\section{Introduction (section 6)}
...
\begin{figure}[!htb]
	\includegraphics[width=\linewidth]{diagnogam.pdf}\caption{Diagnostics of the chosen GAM model, based on the residuals. }\label{fig:diagnogam}
\end{figure}


\begin{center}
\begin{figure}[!htb]
	\includegraphics[width=.75\linewidth]{max_gam.pdf}\caption{Series of annual maxima together with the fitted GAM model (in green). Thicker lines indicate that the increasing is significant for pointwise confidence interval }\label{fig:center_gam}
\end{figure}
\end{center}

\begin{center}
\begin{figure}[!htb]
	\includegraphics[width=.75\linewidth]{post_draws.pdf}\caption{Draws from the }\label{fig:post_draws}
\end{figure}
\end{center}
